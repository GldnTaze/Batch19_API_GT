...
public class C05_BookStorePathMethod {
    ...
    public  void bookStoreGetTest_PathMethod(){
     ...
     //   And the first book isbn should be 9781449325862
        ...
        //2.yolYOL
        String firstBookISBN_2 = response.path("books.isbn[0]");  //butun isbn lerin hepsini alip
     ...
     Assert.assertEquals(firstBookPublisher,"O'Reilly Media");
        ...
        //And the third book isbn should be "9781449337711"
        //And the third book title should be "Designing Evolvable Web APIs with ASP.NET"
        String thirdBookISBN = response.path("books.isbn[2]");
        Assert.assertEquals(thirdBookISBN,"9781449337711");
        String thirdTitle=  response.path("books.title[2]");
        Assert.assertEquals(thirdTitle,"Designing Evolvable Web APIs with ASP.NET");
        ...
        //And the all isbn numbers should have 13 digits
        Assert.assertTrue(allBooksISBNs.stream().anyMatch(n->n.length()==13));
